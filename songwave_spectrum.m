...
% SONGWAVE_SPECTRUM Application M-file for songwave_specgram.fig
%    FIG = SONGWAVE_SPECTRUM launch songwave_specgram GUI.
%    SONGWAVE_SPECTRUM('callback_name', ...) invoke the named callback.

% Last Modified by GUIDE v2.5 13-Jul-2018 14:59:41
...
global fwidthHz filteroption
if isempty(filteroption)
    errordlg('Please enter valid scale option (log or linear).',...
        'Data Input Error', 'modal')
    return
end
if isempty(fwidthHz)
    errordlg('Please enter valid filter_width (in Hz).',...
        'Filter Width Missing','modal');
    return;
end
global rawDS
if isempty(rawDS)
...
global stimsamprate initialFreq endFreq sDim
sDim='1-D';
...
for ii=1:numfiles
    ...
    [yy, xx] = do_cached_calc('ComplexSpectrum',input, floor(fs/ampsamprate),...
    ...
    if ~isempty(the_checksum)
        ...
        %disp(the_checksum);
%         function_text = load_function_text('ComplexSpectrum');
%         strfpak_version = 'unknown';
%         disp(checksum(function_text,strfpak_version,input, floor(fs/ampsamprate),...
%         floor(1/(2*pi*fwidthHz)*6*fs),fs));
%     disp(num2str(input(1:20)));
%     disp(' ');
    end
    stimsamprate = fs;
...
add_to_STRFPAK_script('songwave_spectrum.m','preprocess');
