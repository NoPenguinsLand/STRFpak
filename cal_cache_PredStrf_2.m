...
for k = 1:length(predDS)     % loop through all data files
    ...
    if matchflg == 1
        ...
        if isempty(index_to_use)
            if size(forwardJNstd,3) == 1
                forwardJN_s(:,:) = fast_filter_filter(forward, forwardJNstd, stdfilt);
            else
                forwardJN_s(:,:) = fast_filter_filter(forward(:,:,1), squeeze(mean(forwardJNstd,3)), stdfilt);
            end
        else
